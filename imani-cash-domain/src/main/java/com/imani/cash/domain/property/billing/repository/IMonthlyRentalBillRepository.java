...
public interface IMonthlyRentalBillRepository extends JpaRepository<MonthlyRentalBill, Long> {
    ...
    @Query("Select monthlyRentalBill From MonthlyRentalBill monthlyRentalBill Where monthlyRentalBill.userRecord = ?1")
    public List<MonthlyRentalBill> getAllUserMonthlyRentalBill(UserRecord userRecord);

    @Query("Select monthlyRentalBill From MonthlyRentalBill monthlyRentalBill Where monthlyRentalBill.userRecord = ?1 and monthlyRentalBill.rentalMonth = ?2")
